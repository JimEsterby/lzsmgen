...
: Fl_Box(x, y, w < minimum_size? minimum_size : w, h < minimum_size? minimum_size : h, name)
{
    box(FL_ROUNDED_BOX);
...
: Fl_Box(x, y, minimum_size, minimum_size, name)
{
    box(FL_ROUNDED_BOX);
    align(Fl_Align(FL_ALIGN_TOP|FL_ALIGN_INSIDE));
    color((Fl_Color)215);  // light yellow
...
int StatePict::mouse_loc() const
{
    ...
    if (m_x < x() + border_width)  // Close to the left
    {
        ...
        else if (m_y > x() + h() - border_width)  // Close to bottom
        {
            result = bottom_left;
        }
        else  // (Center) left
    ...
    else if (m_x > x() + w() - border_width)  // Close to the right
    {
        ...
        else if (m_y > x() + h() - border_width)  // Close to the bottom
        {
            result = bottom_right;
        }
        else  // (Center) right
